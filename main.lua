...
local function setupAssets()
    local rootFolder = "OrionUI"
    local assetsFolder = rootFolder .. "/assets"
    ...
    return assetsFolder .. "/font.ttf"
end

local fontFile = setupAssets()
...
-- Font - Using Code font with fallback
local function GetFont()
    -- Try custom font file first
    local ok1, customFont = pcall(function()
        if isfile and getcustomasset and isfile(fontFile) then
            return Font.new(getcustomasset(fontFile), Enum.FontWeight.SemiBold, Enum.FontStyle.Normal)
        end
    end)
    
    if ok1 and customFont then
        return customFont
    end
    
    -- Try built-in asset ID with pcall protection
    local ok2, font = pcall(function()
    ...
    -- Ultimate fallback - Code font (always available)
    return Font.new(Enum.Font.Code, Enum.FontWeight.SemiBold, Enum.FontStyle.Normal)
end
